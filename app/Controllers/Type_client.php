...
class Type_client extends BaseController
{
    public $_db;
    public $Eglise;
    public $Class='Type_client';
    public $table='type_client';
    public $Champ=[
        'TYPE_CLIENT'=>'text',
        'NOMBRE_ENFANT'=>'number',
       
    ];
    public $id='ID_TYPE_CLIENT';
    public $Code='CODE_TYPE_CLIENT';
     // link action 
    public $link_Add='Type_client/Add_news';
     public $link_edit='Type_client/Edite_news';
     public $link_update='Type_client/Update_news';

     public $link_delete='Type_client/Delete_news/';
     // link de form et liste
     public $link_liste='Type_client/liste_news';
     public $link_form='Type_client/';
     // 
...
public function getLiaison($table,$Where,$id,$value)
{
        ...
        $i=1;
             ...
             foreach ($getListe as $key) 
             {
                ...
                <tr>
                    ...
                    <td>'.$key->NOMBRE_ENFANT.'</td>
                  

                   
                    <td>'.$this->btn_action($key->CODE_TYPE_CLIENT).'</td>
                </tr>
            ';
            $i++;
             }
    ...
    public function Add_news(){
         ...
         $TYPE_CLIENT=htmlspecialchars($this->request->getPost('TYPE_CLIENT'));
         $NOMBRE_ENFANT=htmlspecialchars($this->request->getPost('NOMBRE_ENFANT'));
         $CODE_TYPE_CLIENT='TYC_'.uniqid((md5('CODE_TYPE_CLIENT')));

         $data=['TYPE_CLIENT'=>$TYPE_CLIENT,'NOMBRE_ENFANT'=>$NOMBRE_ENFANT,'CODE_TYPE_CLIENT'=>$CODE_TYPE_CLIENT];
   ...
   public function Delete_news($id_delete){
    ...
    function Update_news(){
        $id=htmlspecialchars($this->request->getPost('id'));
         ...
         $data=['TYPE_CLIENT'=>$TYPE_CLIENT,'NOMBRE_ENFANT'=>$NOMBRE_ENFANT];
    ...
    function formulaire_edite($code){
        ...
        return'
         <input type="hidden"  name="id" value="'.$code->ID_TYPE_CLIENT.'">

        <label for="text" style="color: black; font-size: 18px;"><b>TYPE_CLIENT</b></label>
            <input type="text"  name="TYPE_CLIENT" value="'.$code->TYPE_CLIENT.'" class="form-control">

            <label for="text" style="color: black; font-size: 18px;"><b>NOMBRE_ENFANT</b></label>
            <input type="text"  name="NOMBRE_ENFANT"value="'.$code->NOMBRE_ENFANT.'"class="form-control" >
